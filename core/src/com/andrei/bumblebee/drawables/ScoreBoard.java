...
public class ScoreBoard extends GenericDrawable{
    ...
    private Position position = Position.Position_Top;
    private int linesOfText = 1;  //number of lines of text (used to compute the board height)
    ...
    public void setValues(long steps, long total){
        this.steps = steps;
        this.total = total;
    }

    private String getFirstText(){
        return strTotal + " : " + total;
    }

    private String getSecondText(){
        return strSteps  + " : " + steps;
    }
    ...
    public void render () {
        ...
        poly.draw(polyBatch);
        if(position == Position.Position_Bottom){
            font.draw(polyBatch, getFirstText(), 5, computeBoardHeight());
            font.draw(polyBatch, getSecondText(), Gdx.graphics.getWidth() / 2, computeBoardHeight());
        } else {
            font.draw(polyBatch, getFirstText(), 5, Gdx.graphics.getHeight());
            font.draw(polyBatch, getSecondText(), Gdx.graphics.getWidth() / 2, Gdx.graphics.getHeight());
        }
        polyBatch.end();
